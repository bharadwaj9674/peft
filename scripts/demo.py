"""
Demo script for cross-modal retrieval with actual Flickr8k dataset integration.
"""
...
from src.data.preprocessing import process_audio, process_image
        ...
        with torch.no_grad():
            for batch_idx in tqdm(range(num_batches), desc="Computing gallery embeddings"):
                ...
                if text_batch:
                    ...
                    # Get text embeddings
                    batch_text_embeddings = model.encode_text(input_ids, attention_mask)
                ...
                if valid_audio_inputs:
                    ...
                    batch_audio_embeddings = model.encode_audio(audio_values)
                ...
                if valid_image_inputs:
                    ...
                    batch_image_embeddings = model.encode_image(pixel_values)
            ...
            if st.button("Search with Audio", key="audio_search"):
                with torch.no_grad():
                    ...
                    try:
                        ...
                        # Get audio embedding
                        audio_embedding = model.encode_audio(input_values)
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="./data/flickr8k_audio", help="Path to dataset root")
    ...
    # Parse args via Streamlit's way (if running as a Streamlit app)
    try:
        import streamlit as st
        args = st.sidebar.text_input(
            "Command line args", 
            "--config configs/minimal.yaml --checkpoint outputs/minimal/checkpoints/best.pt --data_root ./data/flickr8k_audio --gallery_limit 50"
        )
